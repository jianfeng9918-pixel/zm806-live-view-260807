...
- Stage 1 refreshes use two lightweight BI exports named in `docs/BI刷新操作手册.md`: exactly five store/region sheets from dashboard 15 and exactly two functional/group-control sheets from dashboard 16. Never use screenshots, visible rows, or BI's full-card "select all" export as the data source. Headquarters Today and Cumulative amounts include store plus functional totals; functional data must never enter store/region rankings, fastest lists, attention lists, or directories. The public page polls for a new `report.json` once per minute while retaining the user's current view.
